// Imports js functions from modules to pull and calculate data from salaryData
...
// This function creates a new <section> with radio inputs based on the data provided in the labels array.
function renderInputButtons(labels, groupName) {
  ...
  labels.forEach((label) => {
    // For each label, creates the radio input element.
    ...
    // Creates a label for that radio input element.
    let labelElement = document.createElement("label");
    labelElement.setAttribute("for", label);
    labelElement.innerText = label;
    divElement.appendChild(labelElement);

    // Updates the results when the input is selected.
